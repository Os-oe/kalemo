...
with server(8797) as base, sync_playwright() as p:
    ...
    def desktop():
        ...
        page.wait_for_selector('.overlay [data-k="no"]', timeout=8000)
        S.check('Onboarding: Angebot „Jetzt in die Luft?" nach dem ersten Treffer (Desktop)', True)
        page.click('.overlay [data-k="no"]')
        t_day = time.time()
